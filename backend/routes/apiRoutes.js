...
const router = express.Router();  //it is used to define and group routes in a modular way and define middleware functions that are specific to those routes
...
router.get("/jobs", jwtAuth, (req, res) => {
  ...
  if (req.query.q) {
    findParams = {
      ...findParams,
      title: {
        $regex: new RegExp(req.query.q, "i"),     //"i" flag, which makes the search case-insensitive.
      },
    };
  }
  ...
  let arr = [       //array of pipeline stages 
    {
      $lookup: {
        from: "recruiterinfos",
        localField: "userId",
  ...
  if (Object.keys(sortParams).length > 0) {
    arr = [
      ...
      { $unwind: "$recruiter" },
      { $match: findParams },  //deconstructs the array of documents / flattening the arr of docs.
      {
        $sort: sortParams,
      },
    ];
